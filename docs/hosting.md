...
3. **Budgets are the contract with the box.** `/etc/infra-sim/console.yaml`
   (fleet size, live simulations, disk, TTL); refusals quote it, and tuning it
...
never in the unit file, on a command line, or in the repo.
...
that fleet's dashboards and trigger its API, so treat the port range
...
design and carrying counts and timestamps only, for any uptime monitor:
