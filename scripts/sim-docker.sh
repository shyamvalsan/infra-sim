...
# Start exporters on a manual `telemetry start` (create overrides explicitly).
EXPORTERS="${EXPORTERS:-yes}"
...
cmd_build() {
  ...
  run cp "$REPO/docker/entrypoint.sh" "$ctx/entrypoint.sh"
...
cmd_create() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      # (room IDs may also arrive via INFRA_SIM_CLAIM_ROOMS: they are
      # credentials and the console must not put them on argv, where ps
      # reads them for the whole create. The flag stays for CLI use.)
      --url) url="$2"; shift 2 ;;
  ...
  EXPORTERS="$exporters" cmd_telemetry "$name" start || warn "telemetry did not start; $0 telemetry $name start to retry"
...
cmd_telemetry() {
  ...
  case "$action" in
    start)
      ...
      # Clear any deliberate-stop marker: starting means supervise again.
      docker exec "$container" sh -c "rm -f ${env%/*}/.telemetry-off 2>/dev/null || true"
      ...
      # Prometheus exporters for the application tier, scraped by netdata's
      # own go.d collector. The scrape config (vnode registry + jobs, one
      # shared app so the fleet's charts aggregate) is generated by the plugin
      # itself - bash never re-implements the naming, because chart IDs embed
      # job names.
      # The supervisor (docker/entrypoint.sh) restarts telemetry after a
      # container restart; exporters are conditional, so their intent is a
      # marker in the payload, which survives restarts.
      if [ "$EXPORTERS" = yes ]; then
        touch "$dir/.exporters-on"
      fi
      if [ "$EXPORTERS" = yes ]; then
        local conf_out
        conf_out="$(docker exec "$container" \
          sh -c "$plugin --exporter-config /etc/netdata --environment $env" 2>/dev/null || true)"
        if echo "$conf_out" | grep -q "job(s)"; then
          docker exec -d "$container" sh -c \
            "$plugin --exporters --environment $env >/tmp/infra-sim-exporters.log 2>&1"
          # go.d reads its vnode registry once, at startup, so it gets one
          # restart to pick the new config up. The agent respawns it within
          # seconds; no netdatacli command exists for this.
          #
          # pkill with bracket/anchor patterns on purpose: the old
          # ps|grep|awk|kill pipelines matched the `sh -c` wrapper running
          # them (its own cmdline contains the pattern text), and the
          # plugin-restart variant's awk escaping was wrong so it never fired
          # at all - the label-restore ordering below has been a silent no-op
          # since it shipped (found by the multi-agent review, verified by an
          # isolated expansion test). The bracket form cannot match its own
          # literal; the anchor cannot match a cmdline starting with `sh`.
          docker exec "$container" sh -c "pkill -f '[g]o.d.plugin' || true"
          ...
          docker exec "$container" sh -c "pkill -f '^$plugin 1' || true"
        else
    ...
    stop)
      ...
      # Anchored on the plugin path: an unanchored pattern also matches the
      # `sh -c` wrapper running the pkill itself, which then dies mid-pkill,
      # docker exec returns 143, and set -e aborts the script before the
      # remaining processes are stopped - telemetry stop has silently never
      # stopped more than the logs writer.
      # The marker outlives the container: the supervised entrypoint reads it,
      # so a restart cannot undo a deliberate stop.
      docker exec "$container" sh -c "touch ${env%/*}/.telemetry-off" \
        || warn "could not mark telemetry stopped; the supervisor may restart it"
      docker exec "$container" sh -c "pkill -f '^$plugin --logs' || true"
      docker exec "$container" sh -c "pkill -f '^$plugin --otlp' || true"
      docker exec "$container" sh -c "pkill -f '^$plugin --exporters' || true"
    ...
    status)
      ...
      local exp
      exp="$(docker exec "$container" sh -c \
        "ps -eo args | grep -- '--exporters' | grep -v grep | head -1" 2>/dev/null)"
      if [ -n "$exp" ]; then
        echo "  exporters : serving (app charts aggregate under prometheus.infra_sim_app.*)"
      else
        echo "  exporters : not running"
      fi
      ;;
