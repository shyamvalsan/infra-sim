...
infrastructure: a lot of it, behaving badly on demand.
...
to grade an answer: did the alert fire for the right reason, and did the
investigation find the cause or a coincidence?

**Breadth is otherwise unobtainable.** Nobody stands up real instances of hundreds
of technologies, in every combination, at the size others run them, whether to
test an integration or to practise on one.
...
engine really evaluates its alarms and raises alerts, and Netdata AI really
investigates and reaches its own conclusions.

Nothing downstream of data injection is ever scripted, canned or mocked. There
are no prepared AI answers or fake alert states, and no dashboard is ever drawn
to look busy. When you see Netdata find a root cause here, Netdata found it;
that is the only reason watching it is worth anything.
...
On macOS, **no `sudo`**: the console runs unprivileged so it can reach
Docker Desktop's user-scoped socket, and it runs in a container rather than
on the host:
...
4. **Claim it into Netdata Cloud** (optional) with a token from
   *Cloud → Connect Nodes*, and a room id if you want one. The agent is fresh and
   ...
   touched.
5. **Watch the product work on it.** ML trains on it and the health engine alerts
   on it; Netdata AI investigates it. See the rule above.
6. **Trigger an incident** on the Run tab: start a scenario, then escalate and
   resolve it.
...
and the shared-host rules in-page, and a first-run pointer shows the
workflow until dismissed. The console is built to be **hosted** on a shared machine: every action names
...
and a systemd unit; **[docs/hosting.md](docs/hosting.md)** is that runbook, and
...
way real fleets carry them, such as `environment` and `team`, authored per fleet
...
names, but its signals move independently, which a real system's do not. The
...
- **[docs/QUICKSTART.md](docs/QUICKSTART.md)**: the shortest path from nothing to
  ...
  attempt.
- **[docs/operating.md](docs/operating.md)**: the reference for containers, the
